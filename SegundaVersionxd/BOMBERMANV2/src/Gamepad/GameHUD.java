...
public class GameHUD {
    ...
    GameHUD() {
        ...
        this.ballom = new Ballom[4];
        this.enemyInfo = new BufferedImage[4];
    ...
    void init() {
        ...
        //Enemigo
        this.enemyInfo[0] = new BufferedImage(infoWidth, height, BufferedImage.TYPE_INT_RGB);
    }
    ...
    void assignEnemy(Ballom ballom, int enemyID) {
        this.ballom[enemyID] = ballom;
    }
    ...
    void drawHUD() {
        ...
        Graphics[] enemyGrafics={
                this.enemyInfo[0].createGraphics()
        };
        // Clean enemys
        enemyGrafics[0].clearRect(0, 0, enemyInfo[0].getWidth(), enemyInfo[0].getHeight());
        enemyGrafics[0].setColor(Color.WHITE);
        for (int i = 0; i < enemyGrafics.length; i++) {
            Font font = new Font("Courier New", Font.BOLD, 24);
            // Draw border and sprite
            enemyGrafics[i].drawRect(1, 1, this.enemyInfo[i].getWidth() - 2, this.enemyInfo[i].getHeight() - 2);
            enemyGrafics[i].drawImage(this.ballom[i].getBaseSprite(), 0, 0, null);
            // Draw score
            enemyGrafics[i].setFont(font);
            enemyGrafics[i].setColor(Color.WHITE);
            // Dispose
            enemyGrafics[i].dispose();
        }
